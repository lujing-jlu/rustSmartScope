...
fn main() {
    println!("╔══════════════════════════════════════════════════════╗");
    println!("║       YOLOv8 RKNN Inference Benchmark (100 iter)    ║");
    println!("╚══════════════════════════════════════════════════════╝\n");
    ...
    println!("Step 3: Preprocessing image (once)...");
    let start_preprocess = Instant::now();
    let buffer = image_to_rknn_buffer(&img, model_w, model_h);
    ...
    // Step 4: Warmup run (1 iteration)
    println!("Step 4: Warmup run...");
    let _ = detector.detect(&buffer).expect("Warmup failed");
    println!("✓ Warmup completed\n");

    // Step 5: Benchmark (100 iterations)
    println!("Step 5: Running benchmark (100 iterations)...");
    ...
    for i in 0..100 {
        ...
        let results = detector.detect(&buffer).expect("Inference failed");
        ...
        // Store detection count from first iteration
        if i == 0 {
            println!("\n  First iteration detected {} objects", results.len());
        }
    }
    println!("\n✓ Benchmark completed!\n");
    ...
    println!("  Letterbox + Resize: {:.2}ms", preprocess_time.as_secs_f64() * 1000.0);
    ...
    println!("  Total (100x):  {:.2}ms", total_time.as_secs_f64() * 1000.0);

    println!("\nThroughput:");
    println!("  FPS (avg):     {:.2}", 1000.0 / avg_ms);
    println!("  Images/sec:    {:.2}", 1000.0 / avg_ms);

    println!("\nBreakdown:");
    println!("  The {:.2}ms includes:", avg_ms);
    println!("  - RKNN inference (NPU execution)");
    println!("  - Post-processing (NMS, coordinate transform)");
    println!("  - Memory copies and format conversions");
    println!("\n  Preprocessing ({:.2}ms) is done once per image",
             preprocess_time.as_secs_f64() * 1000.0);
    println!("  and is NOT included in the per-iteration timing.\n");

    println!("╔══════════════════════════════════════════════════════╗");
    println!("║                       Summary                        ║");
    println!("╚══════════════════════════════════════════════════════╝");
    println!("  Model input:     {}x{} pixels", model_w, model_h);
    println!("  Iterations:      100");
    println!("  Avg per iter:    {:.2}ms", avg_ms);
    println!("  Preprocess:      {:.2}ms (one-time)", preprocess_time.as_secs_f64() * 1000.0);
    println!("  Total pipeline:  {:.2}ms per image\n",
             preprocess_time.as_secs_f64() * 1000.0 + avg_ms);
