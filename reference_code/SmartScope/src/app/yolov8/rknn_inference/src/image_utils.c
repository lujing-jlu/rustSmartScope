...
{
    const char *_ext = strrchr(entry->d_name, '.');
    return (_ext && (!strcmp(_ext, ".jpeg") || !strcmp(_ext, ".jpg") || !strcmp(_ext, ".png") || 
                      !strcmp(_ext, ".bmp") || !strcmp(_ext, ".JPEG") || !strcmp(_ext, ".JPG") || 
                      !strcmp(_ext, ".PNG") || !strcmp(_ext, ".BMP")));
}
...
int load_image(const char* path, image_buffer_t* image)
{
    fprintf(stderr, "请使用OpenCV加载图像\n");
    return -1;
}

int save_image(const char* path, image_buffer_t* image)
{
    fprintf(stderr, "请使用OpenCV保存图像\n");
    return -1;
}
...
{
    return save_image(path, (image_buffer_t*)img);
}

int load_jpeg(const char* file_path, unsigned char** img_data, int* width, int* height, int* comp)
{
    fprintf(stderr, "使用OpenCV加载JPEG图像, 请在实际项目中实现此函数\n");
    return -1;
}